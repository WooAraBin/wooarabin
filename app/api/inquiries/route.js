...
async function generateInquiryNumber() {
  ...
  const { count } = await getSupabaseAdmin()
    ...
    .like('inquiry_number', `WB-${dateStr}-%`)
...
export async function POST(req) {
  ...
  await resend.emails.send({
    ...
    html: `
      ...
      <p style="color:#888;font-size:13px">검토 후 1~3 영업일 내에 이메일로 연락드립니다.</p>
    `,
  }).catch(e => console.error('User email error:', e))

  return NextResponse.json({ id: data.id, inquiryNumber }, { status: 201 })
}
